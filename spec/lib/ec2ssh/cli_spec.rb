...
END
  ...
  describe '#update' do
    context 'no dns_name_key option' do
      before do
        silence(:stdout) do
...
END
    ...
    context '"dns_name" in dns_name_key option' do
      before do
        silence(:stdout) do
          cli.start %W[init --path #{ssh_config_path} --dotfile #{dotfile_path}]
          cli.start %W[update --path #{ssh_config_path} --dotfile #{dotfile_path} --dns_name_key dns_name]
        end
      end
...
END
    ...
    context '"private_dns_name" in dns_name_key option' do
      before do
        Ec2ssh::Hosts.tap do |cls|
          cls.class_eval do
            def all dns_name_key
              [
                {:host => 'db-01.ap-northeast-1', :dns_name => 'ec2-1-1-1-1.ap-northeast-1.compute.internal'},
                {:host => 'db-02.ap-northeast-1', :dns_name => 'ec2-1-1-1-1.ap-northeast-1.compute.internal'},
              ]
            end
          end
        end
        silence(:stdout) do
          cli.start %W[init --path #{ssh_config_path} --dotfile #{dotfile_path}]
          cli.start %W[update --path #{ssh_config_path} --dotfile #{dotfile_path} --dns_name_key private_dns_name]
        end
      end
